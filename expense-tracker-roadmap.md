...
- **`0.9.6` — soft-delete + trash for expenses and settlements.** `DELETE /expenses/{id}` now soft-deletes (payer or admin) instead of hard-deleting: `expenses.deleted_at` / `deleted_by_id` columns (new household-stream migration `e1f2a3b4c5d6`, same two on `settlements`). A trashed row drops out of history, `GET /expenses`/`/settlements`, and the balance math immediately (`compute_net_balances` filters `deleted_at IS NULL`), and is restorable by an admin via `POST /{id}/restore`. Settlements got a delete path for the first time (`DELETE /settlements/{id}` — either party or an admin). Trash is admin-only (`?include_deleted=true`), rendered as a collapsible "recently deleted" section in History with Restore buttons. An opportunistic purge (`purge_expired_trash`, run from `GET /balances` and `GET /expenses`) hard-deletes rows older than `settings.trash_retention_days` (default 30) — no scheduler. `?purge=true` on either DELETE is an admin's immediate hard-delete. Export/restore carries the new columns through the raw-file copy untouched; the restore summary counts live rows only.

**Test count: 94** (82 through 0.9.5, + 12 in `test_soft_delete.py`). Two `test_backup_restore` failures and two `test_migrations` teardown errors show up on Windows only — `PermissionError [WinError 32]` releasing a temp `.db`, an environment quirk; all green in the Linux/Docker container.
